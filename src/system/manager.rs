use artisan_middleware::{dusa_collection_utils::{errors::{ErrorArrayItem, Errors}, types::PathType}, git_actions::{GitAuth, GitCredentials}, portal::ManagerData, state_persistence::AppState};
use simple_comms::network::utils::get_local_ip;

use crate::applications::child::{APP_STATUS_ARRAY, CLIENT_APPLICATION_ARRAY, SYSTEM_APPLICATION_ARRAY};
...
pub async fn get_manager_data(state: &mut AppState) -> Result<ManagerData, ErrorArrayItem> {
    let manager_version = state.version.clone();
            ...
            let git_credentials: GitCredentials =  if let Some(config) = &state.config.git {
                let cred_array: Vec<GitAuth> = GitCredentials::new_vec(Some(&PathType::Str(config.credentials_file.clone().into()))).await?;
                let credentials: GitCredentials = GitCredentials {
                    auth_items: cred_array,
                };
                credentials
            } else {
                return Err(ErrorArrayItem::new(Errors::ConfigParsing, "Failed to parse the git repos file on the manager"))
            };
            ...
            let system_warning_count = {
                ...
                for system in system_array.clone() {
                    if let Some(state) = system.1.state {
            ...
            let client_warning_count = {
                ...
                for client in client_array.clone() {
                    if let Some(state) = client.1.state {
                        let count = state.error_log.len();
                        num += count   
                    }
                }
            ...
            let identity = if let Some(id) = load_identifier().await {
                ...
                return Err(ErrorArrayItem::new(Errors::AuthenticationError, "No identity data found on system"));
            };
